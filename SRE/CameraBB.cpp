...
namespace SRE
{
	Camera* Camera::m_pCurrentCamera = nullptr;
	...
	void Camera::Reset() {
		camera_position = glm::vec3(0.f, 0.0f, -10.f);
		camera_up = glm::vec3(0, 1, 0);
	}
	...
	void Camera::Move2D(int x, int y) {
		...
		//if (move_camera) {
			ChangeHeading(m_fSensitivity * mouse_delta.x);
			ChangePitch(m_fSensitivity * mouse_delta.y);
		//}
	...
	{
		SRE::L_INFO(xChange, yChange);
		Move2D(xChange, yChange);
	}

	void Camera::SetSensitivity(float _fSensitivity)
	{
